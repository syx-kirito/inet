...
void TCP_lwIP::handleIpInputMessage(Packet *packet)
{
    ...
    const auto& x = packet->peekAt(packet->getHeaderPopOffset(), bit(packet->getBitLength()));     //FIXME KLUDGE
    const auto& bytes = x->peek<BytesChunk>(bit(0), bit(x->getChunkLength()));
